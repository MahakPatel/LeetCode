...
public:
    ...
    //         int maxLen = 0; // Variable to store the maximum length of the
    //         substring int maxFre = 0; // Variable to store the maximum
    //         frequency of any character in the current window

    //         // Iterate through the string with the right pointer
    //         while (right < n) {
    //             // Increment the count for the current character
    //             mp[s[right]]++;
    //             // Update the maximum frequency of any character in the
    //             current window maxFre = max(maxFre, mp[s[right]]);

    //             // Check if the number of characters that need to be changed
    //             exceeds k while ((right - left + 1) - maxFre > k) {
    //                 // Decrease the count of the character at the left
    //                 pointer mp[s[left]]--;
    //                 // Reset maxFre to 0 and find the new maximum frequency
    //                 maxFre = 0;
    //                 for (int i = 0; i < 26; i++) { // Corrected range to 26
    //                 for uppercase letters
    //                     maxFre = max(maxFre, mp[i]); // Update maxFre based
    //                     on current frequencies
    //                 }
    //                 left++; // Move the left pointer to the right
    //             }

    //             // Calculate the length of the current valid window and
    //             update maxLen if ((right - left + 1) - maxFre <= k) {
    //                 maxLen = max(maxLen, right - left + 1); // Update the
