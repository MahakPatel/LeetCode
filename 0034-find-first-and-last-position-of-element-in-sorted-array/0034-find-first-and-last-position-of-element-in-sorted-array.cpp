...
public:
    ...
    //++++++++++++++ using lower bound upper bound concept +++++++++++++++++++
        ...
        temp.push_back(lb);
        int ub = upperBound(nums, nums.size(), target) -1; // so upperbound is always grtaer than the valu so we
                    // last accurance so the last accurance is always less the the upperbound index
        temp.push_back(ub);
