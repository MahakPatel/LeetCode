...
public:
    ...
    //     if (buy) {
    //         // Two choices:
    //         // 1. Buy the stock (-prices[index]) and move to the next index
    //         with
    //         // 'buy' set to 0
    ...
    //     else {
    //         // Two choices:
    //         // 1. Sell the stock (+prices[index]) and move to the next index
    //         // with 'buy' set to 1
    //         // 2. Skip selling and keep 'buy' as 0
    //         profit = max(prices[index] + f(index + 1, 1, prices), // Sell
    ...
    //     return f(0, 1, prices); // Start from index 0 with buying allowed
    // }
    ...
    //     it. if (dp[index][buy] != -1)
    //         return dp[index][buy];

    //     int profit = 0;

    //     if (buy) { // Buying allowed
    //         profit = max(-prices[index] + f(index + 1, 0, prices, dp), // Buy
    //                      f(index + 1, 1, prices, dp) // Skip buying
    //         );
    //     } else { // Selling allowed
    //         profit = max(prices[index] + f(index + 1, 1, prices, dp), // Sell
    //                      f(index + 1, 0, prices, dp) // Skip selling
    //         );
    //     }
