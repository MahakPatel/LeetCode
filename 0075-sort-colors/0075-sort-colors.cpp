...
public:
    void sortColors(vector<int>& nums) {
        ...
        int high =
            n - 1; // Pointer for the boundary of 2s, initially at the end
        ...
        while (mid <= high) {
            if (nums[mid] == 0) { // If the current element is 0
                ...
                mid++; // Simply move the mid pointer to the right, as 1s are in
                       // the middle section
                ...
                // Note: Do not move the mid pointer here because we need to
                // check the new element at mid
            }
        }
    }
