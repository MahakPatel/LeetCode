class DisjointSet {
    vector<int> rank, parent, size; // Vectors for rank, parent, and size of
                                    // each node in the disjoint set
public:
    DisjointSet(int n) {
    ...
    void unionByRank(int u, int v) {
        ...
        if (ulp_u == ulp_v)
            return; // If both have the same parent, no need to unite
        ...
        if (rank[ulp_u] < rank[ulp_v]) {
            ...
            parent[ulp_v] = ulp_u;
            rank[ulp_u]++; // If both trees have the same rank, increment the
                           // rank of the root
        }
    }
...
public:
    int removeStones(vector<vector<int>>& stones) {
        ...
        // Initialize disjoint set with a size large enough to accommodate all
        // rows and columns
        ...
        for (auto it : stones) {
            ...
            int nodeCol = it[1] + maxRow +
                          1; // Offset column index to avoid overlap with rows
        ...
        // Count how many distinct connected components there are by checking
        // the ultimate parent of each stone
        ...
        // The result is the total number of stones minus the number of distinct
        // components
        return n - count;
    }
